...
function listUsers(options: ListUsersOptions = { per_page: 50, page: 0 }) {
	return mockUsers.slice(options.page * options.per_page, options.page * options.per_page + options.per_page);
}
...
export async function GET(request: Request) {
  ...
  const userId = searchParams.get('userId');
	...
	if (!foundUser) {
		 ...
		 */
		await capture('/api/user invariant state, user not found.');
