...
export default function KitchenSinkWithDataPage() {
	const tasks = useMemo(() => {
  ...
  function noop(task: any) {
    return task.id
  }

	function ClickHandlingButton() {
		function handleClickWithData(e: any) {
			const startTime = e.timeStamp;
	
			// Handle some context
      const transformed = tasks.map(noop)
	
			// Finish off with a raF
		...
		return (
			<Button onClick={handleClickWithData}>Button</Button>
		)
	}

  return (
    <Profiler id="kitchen-sink-page-with-data" onRender={(id: string, phase: string, actualDuration: number, baseDuration: number, startTime: number, commitTime: number) => {
